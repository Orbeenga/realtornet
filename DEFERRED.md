...
| DEF-N-NOTIFICATIONS-001 | Missing notification emails — `agency_review → agency_rejected` (agent email with reason), `draft → agency_review` (submission, no notification currently), `agency_review → admin_review` (approval, no notification currently). These fire points are specified in workbook Section 1.6 but not yet wired. **Closed Phase O** — all three fire points wired and tested: `send_submission_notification_email`, `send_agency_approval_notification_email`, `send_property_moderation_email`. Verified at HEAD `2aeddc2`, 11 passing tests in `test_phase_n4_notifications.py`. | N ✗ |

...
class TestPropertyEdgeCases:
    ...
    def test_multiple_soft_deletes_idempotent(self, db: Session, sample_property):
        ...
        # First delete
        deleted1 = property_crud.soft_delete(db, property_id=sample_property.property_id)
        first_deleted_at = deleted1.deleted_at
        
        # Second delete (should not crash)
        deleted2 = property_crud.soft_delete(db, property_id=sample_property.property_id)
        
        # Timestamp should remain the same (or be updated, depending on implementation)
        assert deleted2.deleted_at is not None
